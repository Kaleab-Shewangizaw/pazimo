...
const updateTicketAvailability = async () => {
  ...
  try {
    ...
    for await (const event of cursor) {
      ...
      // Find wave tickets
      const firstWave = ticketTypes.find(
        (t) => t.name === "Regular - First Wave"
      );
      const secondWave = ticketTypes.find(
        (t) => t.name === "Regular - Second Wave"
      );
      const finalWave = ticketTypes.find(
      ...
      if (firstWave && secondWave && finalWave) {
        const firstWaveEnded =
          currentDate > new Date(firstWave.endDate) || firstWave.quantity <= 0;
        const secondWaveEnded =
          currentDate > new Date(secondWave.endDate) ||
