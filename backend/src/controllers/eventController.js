...
// Create event
const createEvent = async (req, res) => {
  const { title, description, category, isPublic, startDate, endDate, startTime, endTime, location, ticketTypes, capacity, tags, organizer, ageRestriction } = req.body;
  ...
  if (ageRestriction && ageRestriction.hasRestriction) {
    eventData.ageRestriction = {
      hasRestriction: true,
      minAge: ageRestriction.minAge ? parseInt(ageRestriction.minAge) : undefined,
      maxAge: ageRestriction.maxAge ? parseInt(ageRestriction.maxAge) : undefined,
    };
  }
...
const buyTicket = async (req, res) => {
  ...
  const isWaveTicket = selectedType.name.includes('Wave') || selectedType.name.includes('First') || selectedType.name.includes('Second') || selectedType.name.includes('Final');
  
  if (isWaveTicket && event.ticketTypes[ticketTypeIndex].quantity === 0) {
    // Find wave tickets
    const firstWave = event.ticketTypes.find(t => t.name === 'Regular - First Wave');
    const secondWave = event.ticketTypes.find(t => t.name === 'Regular - Second Wave');
    const finalWave = event.ticketTypes.find(t => t.name === 'Regular - Final Wave');
    
    // Activate next wave if current wave is sold out
    if (selectedType.name === 'Regular - First Wave' && secondWave && secondWave.quantity > 0) {
      const secondWaveIndex = event.ticketTypes.findIndex(t => t.name === 'Regular - Second Wave');
    ...
    } else if (selectedType.name === 'Regular - Second Wave' && finalWave && finalWave.quantity > 0) {
      const finalWaveIndex = event.ticketTypes.findIndex(t => t.name === 'Regular - Final Wave');
  ...
  for (let i = 0; i < quantity; i++) {
    const ticket = new Ticket({
      ...
      ticketType: selectedType.name,
      price: selectedType.price,
      paymentReference: paymentReference || `TXN-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
const updateEvent = async (req, res) => {
  ...
  if (!title || !description || !category || !startDate || !endDate || !startTime || !endTime || !capacity) {
  ...
  for (const ticket of parsedTicketTypes) {
    if (!ticket.name || !ticket.price || !ticket.quantity) {
      throw new BadRequestError('All ticket fields (name, price, quantity) are required');
    }
    if (ticket.price <= 0 || ticket.quantity <= 0) {
      throw new BadRequestError('Ticket price and quantity must be greater than 0');
    }
  }
  ...
  event.tags = Array.isArray(tags) ? tags : (tags ? tags.split(',').map(tag => tag.trim()) : []);
  
  // Handle age restriction
  if (parsedAgeRestriction && parsedAgeRestriction.hasRestriction) {
    event.ageRestriction = {
      hasRestriction: true,
      minAge: parsedAgeRestriction.minAge ? parseInt(parsedAgeRestriction.minAge) : undefined,
      maxAge: parsedAgeRestriction.maxAge ? parseInt(parsedAgeRestriction.maxAge) : undefined,
    };
  } else {
...
const getAllEvents = async (req, res) => {
  try {
    ...
    if (category) query.category = category;
    if (status) query.status = status;
    if (bannerStatus !== undefined) query.bannerStatus = bannerStatus === 'true';
    if (startDate) query.startDate = { $gte: new Date(startDate) };
    if (endDate) query.endDate = { $lte: new Date(endDate) };
    if (city) query['location.city'] = city;
    if (country) query['location.country'] = country;
    ...
    const organizerIds = events.map(event => event.organizer?._id).filter(Boolean);
    ...
    const eventsWithOrganization = events.map(event => {
      ...
      if (eventObj.organizer) {
        eventObj.organizer.organization = orgMap[eventObj.organizer._id.toString()] || null;
      }
      return eventObj;
    });
...
const updateTicketTypes = async (req, res) => {
  ...
  if (event.organizer.toString() !== req.user.id && req.user.role !== 'admin') {
    throw new BadRequestError('Not authorized to update tickets for this event');
  }
  ...
  try {
    ...
    const formattedEvent = {
      ...
      ticketTypes: event.ticketTypes.map(ticket => ({
        ...
        startDate: ticket.startDate ? new Date(ticket.startDate).toISOString() : null,  // Format startDate if available
...
const getWishlist = async (req, res) => {
  try {
    ...
    const wishlistItems = await Wishlist.find({ userId })
      .populate({
        path: "eventId",
        select: "title startDate endDate location category coverImage ticketTypes",
      })
      .sort({ createdAt: -1 })
    ...
    res.status(200).json({ success: true, data: formattedItems, message: "Wishlist retrieved successfully" })
  } catch (error) {
