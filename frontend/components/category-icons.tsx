...
export default function CategoryIcons() {
  ...
  const getIconForCategory = (categoryName: string) => {
    ...
    if (name.includes("education") || name.includes("workshop"))
    ...
    if (name.includes("festival") || name.includes("party"))
      return <Ticket className="w-8 h-8 text-white" />;
    ...
    if (name.includes("entertainment") || name.includes("show"))
  ...
  const nextSlide = useCallback(() => {
    setCurrentIndex((prev) =>
      prev < categories.length - visibleCards ? prev + 1 : 0
    );
  }, [categories.length, visibleCards]);

  const prevSlide = useCallback(() => {
    setCurrentIndex((prev) =>
      prev > 0 ? prev - 1 : categories.length - visibleCards
    );
  }, [categories.length, visibleCards]);
                ...
                {categories.map((category, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative">
                        <div
                          className={`absolute inset-0 ${colors.bg} rounded-full blur-md opacity-0 group-hover:opacity-20 transition-all duration-300 scale-110`}
                        />
                        <div
