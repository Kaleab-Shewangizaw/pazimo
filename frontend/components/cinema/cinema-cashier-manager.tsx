...
/**
 * Create/list/edit/deactivate a cinema's own counter staff.
 *
 * The cinema twin of components/venue/venue-cashier-manager.tsx. Reused as-is
 * by the cinema owner's self-service page ("/api/cinemas/me") and by the
 * admin dashboard's per-cinema dialog ("/api/cinemas/admin/:cinemaId") — both
 * callers just pass the endpoint base and whichever token authorizes them;
 * the backend routes these under cinemaSelf/adminOnly (see
 * backend/src/routes/cinemaRoutes.js), never cinemaStaff, so a cashier's own
 * token is never accepted here.
 */
export function CinemaCashierManager({
  endpointBase,
  token,
}: {
  endpointBase: string;
  ...
  const [cashiers, setCashiers] = useState<CinemaCashier[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<CinemaCashier | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setCashiers(await fetchCashiers(endpointBase, token));
    } catch (error) {
  ...
  const openEdit = (cashier: CinemaCashier) => {
    setEditing(cashier);
    setForm({
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await updateCashier(endpointBase, editing._id, token, {
        ...
        await createCashier(endpointBase, token, {
          firstName: form.firstName.trim(),
  ...
  const toggleActive = async (cashier: CinemaCashier) => {
    setTogglingId(cashier._id);
    try {
      await updateCashier(endpointBase, cashier._id, token, {
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Counter staff — can sell tickets and concessions, scan/check in, and
            view sales history. They can&apos;t touch halls, movies, showtimes,
            finance, or create more cashiers.
          </p>
        </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ...
                : "Creates a login this cashier can use to sell at the counter."}
            </DialogDescription>
          </DialogHeader>
