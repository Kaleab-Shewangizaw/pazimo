...
/**
 * The Tickets tab: sales-by-ticket-type, then who bought them. Shared between
 * the cinema's own dashboard and the admin's cinema panel — `endpointBase` is
 * the only thing that differs (`/api/cinemas/me` vs `/api/cinemas/admin/:id`).
 */
export default function ShowtimeTicketsPanel({
  token,
  endpointBase,
  showtime,
}: {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      // comfortably covers any real room, so neither of these needs its own
      // pagination control.
      const [ticketRes, concessionRes] = await Promise.all([
        staffGet<CinemaTicket[]>(
          endpointBase,
          `/ticket-sales?showtimeId=${showtime._id}&limit=1000`,
          token
        ),
        staffGet<CinemaConcessionSale[]>(
          endpointBase,
          `/concession-sales?showtimeId=${showtime._id}&limit=1000`,
          token
        ),
      ]);
  ...
  const orders = useMemo(() => groupIntoOrders(tickets, concessions), [tickets, concessions]);

  const filteredOrders = useMemo(() => {
    if (!search.trim()) return orders;
    const q = search.trim().toLowerCase();
    return orders.filter(
      (o) => o.buyerName.toLowerCase().includes(q) || (o.buyerPhone || "").toLowerCase().includes(q)
    );
  }, [orders, search]);
  ...
  const detailTickets = useMemo(
    () =>
      detailOrder ? tickets.filter((t, i) => keyOf(t.paymentReference, t._id, i) === detailOrder.key) : [],
    [tickets, detailOrder]
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {showtime.ticketTypes.map((t) => (
          ...
          >
            ...
            <span className="tabular-nums font-semibold text-gray-900 dark:text-gray-100">
              {money(t.sold * t.price)}
            </span>
          </div>
        ))}
        {showtime.ticketTypes.length === 0 && (
        ...
        <div className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs dark:border-gray-800 dark:bg-gray-950/50">
          ...
          <span className="tabular-nums font-semibold text-gray-900 dark:text-gray-100">
            {money(concessionTotals.revenue)}
          </span>
        </div>
      </div>
                  ...
                  return (
                    <tr key={o.key} className="border-b border-gray-100 dark:border-gray-800/60">
                      <td className="py-2 pr-4">
                        <div className="font-medium text-gray-900 dark:text-gray-100">{o.buyerName}</div>
                        {o.buyerPhone && (
                          <div className="text-xs text-gray-500 dark:text-gray-400">{o.buyerPhone}</div>
                        )}
                      </td>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {detailTickets.map((t) => (
                    ...
                    >
                      <div>
                        <span className="font-mono text-gray-900 dark:text-gray-100">{t.ticketId}</span>
