...
export default function FeaturedEventCard({
  ...
  return (
    <Link href={href} className={cn("block group cursor-pointer h-full", className)}>
      <div className="relative overflow-hidden rounded-2xl mb-4 shadow-sm border border-gray-100 bg-gray-50">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="text-lg font-display font-semibold mb-1 text-white">
            {data.title}
          </p>
      ...
      <div className="flex items-center justify-between px-1">
        <span className="text-sm font-medium text-[#1a2d5a]">{data.priceLabel}</span>
        {showCTA && (
          <Button
            variant="ghost"
            size="sm"
