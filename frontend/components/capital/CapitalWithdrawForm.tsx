...
/**
 * A Pazimo Capital withdrawal request.
 *
 * Posts with `stream: "capital"`, which is what keeps it out of the ticket
 * pool — a loan's principal is credited to its own balance at approval, not
 * earned per sale, and approving this request never touches ticket revenue,
 * the ticket ledger or the admin ticket dashboard. Repayment (a cut of later
 * ticket sales) is automatic and separate; nothing here affects it.
 */
export default function CapitalWithdrawForm({
  token,
  available,
  onSuccess,
}: {
  ...
  const submit = async () => {
    ...
    if (parsedAmount > available) {
      toast.error(`That is more than your Pazimo Capital balance of ${formatEtb(available)}`);
      return;
    }
    if (!payout.bankName) {
    ...
    try {
      ...
      toast.success("Withdrawal requested", {
        description: "Track it under loan history — you'll be notified when it's processed.",
      });
      setAmount("");
    ...
    return (
      <div className="rounded-lg border border-dashed border-gray-300 dark:border-gray-700 px-6 py-12 text-center">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Your Pazimo Capital balance is {formatEtb(available)}. It's credited in full
          the moment an advance is approved.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-xl space-y-5">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <Label htmlFor="capital-withdraw-amount" className="text-sm">
            Amount to withdraw
          </Label>
          <button
            type="button"
        ...
        {parsedAmount > available && (
          <p className="flex items-center gap-1.5 text-xs text-red-600 dark:text-red-400">
            ...
            Amount exceeds your Pazimo Capital balance
          </p>
        )}
      </div>
          ...
          {PAYOUT_METHODS.map((method) => {
            ...
            return (
              <div key={method.id} className="relative">
                <RadioGroupItem
                  value={method.id}
                  id={`capital-payout-${method.id}`}
                  className="peer sr-only"
                />
                <Label
                  htmlFor={`capital-payout-${method.id}`}
                  className={cn(
      ...
      {payout.bankName && (
        <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/30 p-4 space-y-3">
          ...
          <div className="space-y-2">
            <Label htmlFor="capital-holder-name" className="text-sm">
              Full name of the account holder
            </Label>
            <Input
              id="capital-holder-name"
          ...
          <div className="space-y-2">
            <Label htmlFor="capital-account-number" className="text-sm">
              {payout.bankName === "bank"
                ? "Bank account number"
