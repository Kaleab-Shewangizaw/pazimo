...
/**
 * The admin withdrawal queue, for one revenue stream.
 *
 * Ticket and beverage withdrawals are separate queues on separate screens
 * because they are separate balances — but they are the same model, the same
 * approval flow and the same payout mechanics, so they are the same component
...
const STREAM_COPY: Record<
  WithdrawalStream,
  {
    ...
    accent: string
    emptyHint: string
    // Beverage sales are ETB only, so that page has nothing to switch between.
    currencies: ("ETB" | "USD")[]
  }
> = {
  tickets: {
    title: "Ticket Withdrawal Requests",
    subtitle:
      "Payouts drawn from ticket revenue. Beverage takings are settled on their own page.",
    accent: "border-t-red-600",
    emptyHint: "Ticket withdrawal requests for the selected filter will appear here",
    currencies: ["ETB", "USD"],
  },
  beverages: {
    title: "Beverage Withdrawal Requests",
    subtitle:
...
export default function AdminWithdrawalQueue({
  ...
  const fetchWithdrawals = useCallback(async () => {
    try {
      ...
      // Without this the two queues show each other's rows.
      url += `&stream=${stream}`
  ...
  }, [token, page, itemsPerPage, statusFilter, currencyFilter, stream])
  ...
  const filteredWithdrawals = withdrawals.filter((withdrawal) => {
    const organizerName =
      `${withdrawal.organizer?.firstName ?? ""} ${withdrawal.organizer?.lastName ?? ""}`.toLowerCase()
    const email = (withdrawal.organizer?.email ?? "").toLowerCase()
    const searchLower = searchQuery.toLowerCase()
    return organizerName.includes(searchLower) || email.includes(searchLower)
  })
  ...
  const renderBankDetails = (withdrawal: WithdrawalData) => {
    ...
    return (
      <div className="text-sm">
        <div className="font-medium capitalize">{bankName}</div>
        <div>{accountName}</div>
  ...
  const statTiles: { label: string; value: Stats[keyof Stats]; tone: string }[] = [
    { label: "Pending", value: stats.pending, tone: "text-yellow-600 dark:text-yellow-400" },
    { label: "Approved", value: stats.approved, tone: "text-blue-600 dark:text-blue-400" },
    { label: "Completed", value: stats.completed, tone: "text-green-600 dark:text-green-400" },
    { label: "Rejected", value: stats.rejected, tone: "text-red-600 dark:text-red-400" },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black p-6">
      <div className="container mx-auto py-10 p-10 max-w-7xl">
        ...
        {/* Stream totals for the current filter */}
          ...
          {statTiles.map((tile) => (
            ...
            >
              <CardContent className="p-4">
                ...
                <p className={`mt-1 text-xl font-bold tabular-nums ${tile.tone}`}>
                  {formatAmount(tile.value.amount)} {currencyFilter}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {tile.value.count} request{tile.value.count === 1 ? "" : "s"}
                </p>
              </CardContent>
            </Card>
          ))}
        ...
        {/* Withdrawals Table */}
        <Card
          className={`border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-xl border-t-4 ${copy.accent}`}
        >
                      ...
                      >
                        ...
                        <TableCell>
                          <span className="font-semibold text-green-600 dark:text-green-400">
                            ...
                            {withdrawal.currency || currencyFilter}
                          </span>
                          {!!withdrawal.feeAmount && withdrawal.feeAmount > 0 && (
                            <div className="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5">
                              -{formatAmount(withdrawal.feeAmount)} Telebirr fee
                              &middot; pay out{" "}
                              {formatAmount(
                                withdrawal.netAmount ??
                                  withdrawal.amount - withdrawal.feeAmount,
                              )}
                            </div>
                          )}
                        </TableCell>
                        <TableCell>
                          {stream === "beverages" ? (
                            <Badge
                              variant="outline"
                              className="border-amber-300 text-amber-700 bg-amber-50 dark:border-amber-800 dark:text-amber-400 dark:bg-amber-950/30"
                            >
                              Bar takings
                            </Badge>
                          ) : stream === "capital" ? (
                        ...
                        <TableCell>{renderBankDetails(withdrawal)}</TableCell>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
              {!!selectedWithdrawal?.feeAmount &&
                selectedWithdrawal.feeAmount > 0 && (
