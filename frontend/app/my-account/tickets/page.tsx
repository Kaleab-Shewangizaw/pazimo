...
import { Search, Calendar, MapPin, Clock, Ticket, User, QrCode, ChevronLeft, ChevronRight, Download } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const TicketSVG = () => (
  ...
  >
    {/* Ticket background */}
    <rect width="280" height="160" rx="8" fill="white" stroke="#E5E7EB" strokeWidth="2" />
    
    {/* Decorative elements */}
    <path d="M0 40 L280 40" stroke="#E5E7EB" strokeWidth="2" strokeDasharray="4 4" />
    <path d="M0 120 L280 120" stroke="#E5E7EB" strokeWidth="2" strokeDasharray="4 4" />
    
    {/* Left cutout */}
    <path d="M0 30 Q-10 40 0 50" stroke="#E5E7EB" strokeWidth="2" fill="white" />
    <path d="M0 110 Q-10 120 0 130" stroke="#E5E7EB" strokeWidth="2" fill="white" />
    
    {/* Right cutout */}
    <path d="M280 30 Q290 40 280 50" stroke="#E5E7EB" strokeWidth="2" fill="white" />
    <path d="M280 110 Q290 120 280 130" stroke="#E5E7EB" strokeWidth="2" fill="white" />
    
    {/* Ticket icon */}
    <circle cx="40" cy="80" r="20" fill="#EFF6FF" />
    <path
      d="M35 80 L38 83 L45 76"
      stroke="#3B82F6"
      strokeWidth="2"
      strokeLinecap="round"
    ...
    {/* Price tag */}
    <rect x="180" y="65" width="80" height="30" rx="4" fill="#EFF6FF" />
    <text x="220" y="80" textAnchor="middle" fill="#3B82F6" fontSize="12" fontWeight="bold">
      ETB / Birr
    </text>
  </svg>
)
...
export default function TicketsPage() {
  ...
  const [eventImageCache, setEventImageCache] = useState<Record<string, string>>({})
  ...
  const getEventImageSrc = (event: any) => {
    const images: string[] = Array.isArray(event?.coverImages) ? event.coverImages : []
    if (images.length > 0) {
  ...
  const EventImage = ({ event, width, height, className }: { event: any; width: number; height: number; className?: string }) => {
    ...
    useEffect(() => {
      ...
      ;(async () => {
        try {
          const r = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/events/details/${eventId}`)
  ...
  const downloadQRCode = (qrCodeDataUrl: string, ticketId: string, ticketType: string, eventTitle: string) => {
  ...
  const downloadAllEventQRCodes = (group: any) => {
    ...
    try {
      group.tickets.forEach((ticket: any, index: number) => {
        setTimeout(() => {
          downloadQRCode(
            ticket.qrCode, 
            ticket.ticketId, 
      ...
      toast.success(`Starting download of all QR codes for ${group.event.title}...`)
    } catch (error) {
    ...
    const fetchTickets = async () => {
      ...
      if (storedAuth) {
        try {
          ...
          console.log('Auth found - User ID:', userId, 'Token exists:', !!token)
        } catch (e) {
      ...
      try {
        ...
        if (!apiUrl) {
          throw new Error('API URL not configured. Please check NEXT_PUBLIC_API_URL environment variable.')
        }
        ...
        try {
          ...
          throw new Error('Backend server is not reachable. Please make sure the backend is running on port 5000.')
        }
        ...
        const filteredByStatus = allTickets.filter((t: any) => t.status === 'active' || t.status === 'used')
        ...
        for (const t of filteredByStatus) {
          const hasEventObject = !!(t.event && typeof t.event === 'object' && t.event._id)
          if (!hasEventObject) {
        ...
        if (missingEventIds.size > 0) {
          console.log('Fetching missing event details for IDs:', Array.from(missingEventIds))
          try {
            const results = await Promise.all(
              Array.from(missingEventIds).map(async (id) => {
                try {
                  const r = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/events/details/${id}`)
                  const j = await r.json()
        ...
        const groupedTickets = hydratedTickets.reduce((acc: any, ticket: any) => {
          if (!ticket.event || !ticket.event._id) {
            console.warn('Skipping ticket without resolvable event data:', ticket)
            return acc
          }
  ...
  const filteredTickets = tickets
    .filter(ticket => {
      const hasActiveTickets = ticket.tickets.some((t: any) => t.status === 'active')
      const hasUsedTickets = ticket.tickets.some((t: any) => t.status === 'used')
    ...
    .filter(ticket =>
      ...
      ticket.event.location.address.toLowerCase().includes(searchQuery.toLowerCase())
    )
  ...
  const handleNextTicket = () => {
    if (selectedGroup && currentTicketIndex < selectedGroup.tickets.length - 1) {
                ...
                <CardContent className="p-4 sm:p-6">
                  <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
                    <div className="w-full sm:w-24 h-32 sm:h-24 rounded-lg overflow-hidden flex-shrink-0">
                      <EventImage event={group.event} width={96} height={96} className="w-full h-full object-cover" />
                    </div>

                    <div className="flex-1 w-full">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div className="flex-1">
                          <h2 className="text-lg font-semibold">{group.event.title}</h2>
                          <div className="mt-2 space-y-1 text-sm text-gray-500">
                            <p><span className="font-medium">Event Date:</span> {new Date(group.event.startDate).toLocaleDateString()}</p>
                            <p><span className="font-medium">Location:</span> {group.event.location.address}, {group.event.location.city}</p>
                            <p><span className="font-medium">Tickets:</span> {group.tickets.length}</p>
                          </div>
                        </div>
                        <div className="flex flex-row sm:flex-col items-center sm:items-end gap-3">
                          ...
                          <p className="text-lg font-semibold text-primary">{group.tickets.reduce((sum: number, ticket: any) => sum + ticket.price, 0)} ETB</p>
                      ...
                      <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <p className="text-sm font-medium">Event ID: {group.event._id}</p>
                        <Dialog onOpenChange={(open) => {
                          ...
                          <DialogContent className="sm:max-w-[500px] p-4">
                            <DialogHeader>
                              <DialogTitle className="text-lg font-bold">{group.event.title}</DialogTitle>
                            </DialogHeader>

                            <div className="mt-4 flex flex-col items-center">
                              {selectedGroup && selectedGroup.tickets[currentTicketIndex] && (
                                <>
                                  <div className="flex justify-between items-center w-full mb-4">
                                    <p className="text-sm font-medium">Ticket ID: {selectedGroup.tickets[currentTicketIndex].ticketId}</p>
                                    <Badge variant="outline" className="capitalize">
                                      {selectedGroup.tickets[currentTicketIndex].status}
                                    </Badge>
                                  </div>
                                  
                                  <div className="flex justify-center mb-4 relative">
                                    <Image
                                      src={selectedGroup.tickets[currentTicketIndex].qrCode || "/placeholder.svg"}
                                      alt="Ticket QR Code"
                                      width={280}
                                      height={280}
                                  ...
                                  <div className="flex justify-between items-center w-full">
                                    ...
                                    <p className="text-sm text-gray-500">
                                      {currentTicketIndex + 1} of {selectedGroup.tickets.length}
                                    </p>
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={handleNextTicket}
                                      disabled={currentTicketIndex === selectedGroup.tickets.length - 1}
                                    >
                                      Next
