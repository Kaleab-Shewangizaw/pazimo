...
const NAV = [
  { href: "/cinema", label: "Overview", icon: LayoutDashboard },
  { href: "/cinema/programme", label: "Programme", icon: CalendarDays },
  { href: "/cinema/tickets", label: "Tickets", icon: Ticket },
  { href: "/cinema/concessions", label: "Concessions", icon: Popcorn },
  { href: "/cinema/money", label: "Money", icon: Wallet },
];
...
export default function CinemaClientLayout({
  ...
  // first would bounce a signed-in cinema to the login screen on every refresh.
  // Same guard the venue and organizer areas use.
  ...
  // the venue and organizer areas admit them.
  const userRole = user?.role as string | undefined;
  const canAccess = Boolean(
    isAuthenticated && user && (userRole === "cinema" || userRole === "admin")
  );
        ...
        <nav className="container mx-auto max-w-6xl overflow-x-auto px-4">
          <ul className="flex gap-1">
            {NAV.map(({ href, label, icon: Icon }) => {
              // Exact match for the index, prefix for the rest, so /cinema is
              // not highlighted while a child route is open.
              const active =
                href === "/cinema"
                  ? pathname === href
                ...
                <li key={href}>
                  <Link
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap border-b-2 px-3 py-2.5 text-sm transition-colors ${
                      active
                        ? "border-indigo-500 font-medium text-indigo-700 dark:text-indigo-400"
