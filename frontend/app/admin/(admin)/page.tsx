...
export default function AdminDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (!revenueChartResponse.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to fetch revenue chart data"
        );
      }
      ...
      const eventRegistrationsChartResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/charts/event-registrations`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (!eventRegistrationsChartResponse.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to fetch event registrations chart data"
        );
      }
      const eventRegistrationsChartData =
      ...
      if (!withdrawalsResponse.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to fetch withdrawal requests"
        );
      }
  ...
  const handleWithdrawalAction = async (
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/withdrawals/${requestId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `Failed to ${action} withdrawal request`
        );
      }
                  ...
                  <div className="flex items-start gap-3 p-4 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-green-600 mt-1">
                        {stats.activeEvents} events currently running
                      </p>
                    </div>
                  </div>
                </div>
