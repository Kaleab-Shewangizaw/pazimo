...
interface EventStore {
  ...
  createEvent: (eventData: FormData) => Promise<void>;
...
export const useEventStore = create<EventStore>((set, get) => ({
  ...
  toggleSoldOut: async (id: string, isSoldOut: boolean) => {
    set({ isLoading: true, error: null });
    try {
      const authState = useAuthStore.getState();
      if (!authState.isAuthenticated || !authState.token) {
      ...
      set((state) => ({
        events: state.events.map((event) =>
          event._id === id ? { ...event, isSoldOut } : event
        ),
        currentEvent:
          state.currentEvent?._id === id
            ? { ...state.currentEvent, isSoldOut }
            : state.currentEvent,
        isLoading: false,
      }));
      toast.success(`Event marked as ${isSoldOut ? "Sold Out" : "Available"}`);
    } catch (error) {
